...
- (id)initAsName:(NSString *)identifier  from:(SGAnimationScript *)script
{
    ...
    result = [self initWithId: sg_Animations_CreateAnimationNamed(identifier_temp, script->pointer)];
    return result;
}

- (id)initAtIndex:(int)identifier  from:(SGAnimationScript *)script
{
    SGAnimation * result;
    result = [self initWithId: sg_Animations_CreateAnimationWithSound(identifier, script->pointer)];
    return result;
}

- (id)initAtIndex:(int)identifier  from:(SGAnimationScript *)script  withSound:(BOOL)withSound
{
    SGAnimation * result;
    result = [self initWithId: sg_Animations_CreateAnimation(identifier, script->pointer, (withSound ? 1 : 0))];
    return result;
}

- (id)initAsName:(NSString *)identifier  from:(SGAnimationScript *)script  withSound:(BOOL)withSound
{
    char identifier_temp[[identifier length] + 1];
    SGAnimation * result;
    [identifier getCString:identifier_temp maxLength:[identifier length] + 1 encoding:NSASCIIStringEncoding];
    result = [self initWithId: sg_Animations_CreateAnimationNamedWithSound(identifier_temp, script->pointer, (withSound ? 1 : 0))];
    return result;
}
...
{
    SGVector * result;
    result = [SGVector vectorForData:sg_Animations_AnimationCurrentVector(self->pointer)];
    return result;
}

- (void)assignAnimationNamed:(NSString *)name  from:(SGAnimationScript *)script
{
    char name_temp[[name length] + 1];
    [name getCString:name_temp maxLength:[name length] + 1 encoding:NSASCIIStringEncoding];
    sg_Animations_AssignAnimationNamed(self->pointer, name_temp, script->pointer);
}

- (void)assignAnimation:(int)idx  from:(SGAnimationScript *)script
{
    sg_Animations_AssignAnimation(self->pointer, idx, script->pointer);
}

- (void)assignAnimationNamed:(NSString *)name  from:(SGAnimationScript *)script  withSound:(BOOL)withSound
{
    char name_temp[[name length] + 1];
    [name getCString:name_temp maxLength:[name length] + 1 encoding:NSASCIIStringEncoding];
    sg_Animations_AssignAnimationNamedWithSound(self->pointer, name_temp, script->pointer, (withSound ? 1 : 0));
}

- (void)assignAnimation:(int)idx  from:(SGAnimationScript *)script  withSound:(BOOL)withSound
{
    sg_Animations_AssignAnimationWithSound(self->pointer, idx, script->pointer, (withSound ? 1 : 0));
}
...
- (void)drawOnto:(SGBitmap *)dest  using:(SGBitmap *)bmp  pt:(SGPoint2D *)pt
{
    sg_Animations_DrawAnimationOntoDestAtPt(dest->pointer, self->pointer, bmp->pointer, &pt->data);
}
