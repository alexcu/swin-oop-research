...
{
    SGPoint2D * result;
    result = [self initWithPoint2D: sg_Geometry_RandomScreenPoint()];
    return result;
}
...
{
    SGPoint2D * result;
    result = [self initWithPoint2D: sg_Geometry_PointAt(x, y)];
    return result;
}
...
- (SGVector *)createVectorToPoint:(SGPoint2D *)p2
{
    SGVector * result;
    result = [SGVector vectorForData:sg_Geometry_CreateVectorFromPoints(&self->data, &p2->data)];
    return result;
}
