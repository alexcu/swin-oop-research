...
+ (SGBitmap *)drawTextFont:(SGFont *)font  string:(NSString *)str  textColor:(color)clrFg  backgroundColor:(color)backgroundColor
{
    char str_temp[[str length] + 1];
    SGBitmap * result;
    [str getCString:str_temp maxLength:[str length] + 1 encoding:NSASCIIStringEncoding];
    result = [SGBitmap createWithId:sg_Text_DrawTextToBitmapAtPointWithFontNamedAndSize(font->pointer, str_temp, clrFg, backgroundColor)];
    return result;
}
